...
class RandomCut(nn.Module):
    # Augmentation technique that randomly cuts the start and end of the audio
...
class WakeWordData(torch.utils.data.Dataset):
    ...
    def __init__(self, data_json, sample_rate = 8000, valid = False):
        ...
        if valid:
            ...
            self.audio_transform = nn.Sequential(get_featurizer(sample_rate), SpecAugment(rate=0.5))
